...
{
	$search = $_POST['search'];
	$people_sql = mysqli_query($conn,"SELECT * FROM user WHERE name LIKE '%$search%'");
	$posts_sql = mysqli_query($conn,"SELECT posts.*,user.name,user.photo FROM posts INNER JOIN user ON posts.user_id=user.id WHERE posts.title LIKE '%$search%' OR posts.description LIKE '%$search%'");
	$data="";
	$data.='<ul class="people list-group mt-2">';
